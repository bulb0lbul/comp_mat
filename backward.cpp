// g++ -std=c++17 -O2 backward_4.cpp -I/opt/homebrew/include/eigen3 -I/Users/yanaprokopovich/Downloads/sofa/20231011/c/src -L/Users/yanaprokopovich/Downloads/sofa/20231011/c/src -lsofa -o backward_4
...
// -----------------------------------------------------------------------------
// Физические и численные константы (единицы: СИ, если не указано иного)
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
const double EARTH_RADIUS = 6378137.0;    // Экваториальный радиус Земли [m]
const double EARTH_OMEGA = 7.2921150e-5;  // Угловая скорость вращения Земли [rad/s]
...
const double MAX_STEP_NORM = 1e7;               // Максимальная норма шага Δβ (например 10 000 км = 1e7 m)

// Регуляризация (Tikhonov) для плохой обусловленности
const double TIKHONOV_LAMBDA = 1e-10;   // Базовый параметр регуляризации
const double MAX_CONDITION_NUMBER = 1e12; // Порог числа обусловленности для усиления регуляризации
...
struct Mat3x3 {
    ...
    // По умолчанию инициализируем единичной матрицей — удобное начальное значение
    Mat3x3() {
        for (int i = 0; i < 3; ++i)
            for (int j = 0; j < 3; ++j)
    ...
    // Сложение, вычитание и умножение на скаляр — все поэлементно
    ...
    // Умножение матрицы на вектор (классическое линейное отображение)
    ...
    // Транспонирование — часто используется для преобразований и проверок
...
struct Object {
    ...
    double mass;      // Масса [kg]
    std::string name; // Читаемое имя (для отладки)
...
// -----------------------------------------------------------------------------
// Прототипы функций (чтобы можно было использовать функции в любой последовательности)
// -----------------------------------------------------------------------------
...
// positions: вектора положений всех тел (в том же порядке, что и objects)
// -----------------------------------------------------------------------------
std::pair<std::vector<Vec3d>, Mat3x3> compute_accelerations_and_jacobian(
    ...
    std::vector<Vec3d> accelerations(n); // рез-т ускорений для каждого тела
...
// -----------------------------------------------------------------------------
// Кубический интерполятор Эрмита для траекторий (позиции) и для матриц чувствительности
// - интерполяция применяется при вычислении состояний в произвольные времена (например, для наблюдений)
// - внутри используется центральная разность для вычисления касательных (tangent)
// -----------------------------------------------------------------------------
class CubicHermiteInterpolator {
private:
    std::vector<Vec3d> trajectory; // точки траектории
...
// Особый случай: направление почти вдоль оси z рассматривается отдельно для устойчивости.
// -----------------------------------------------------------------------------
Eigen::Matrix<double, 2, 3> compute_geometric_jacobian_normalized(const Vec3d& direction_unit) {
    ...
    // ∂RA/∂x, ∂RA/∂y — из atan2(y,x)
    J(0, 0) = -y / rho2;
    J(0, 1) =  x / rho2;
    J(0, 2) = 0.0;

    // ∂DEC/∂(x,y,z) — устойчивое выражение через atan2(z, rho)
    double denom = rho2 + z*z; // для единичного вектора denom==1, но оставляем общую форму
    J(1, 0) = -x * z / (rho * denom);
    J(1, 1) = -y * z / (rho * denom);
    J(1, 2) = rho / denom;
...
// Этот якобиан необходим при дифференцировании направления, полученного из топоцентрика
// -----------------------------------------------------------------------------
Eigen::Matrix3d compute_normalization_jacobian(const Vec3d& r) {
    ...
    if (r_len < 1e-12) {
        J_norm.setIdentity(); // грубая защита: для нулевого вектора возвращаем I
        return J_norm;
    }
...
// -----------------------------------------------------------------------------
// Заготовка (упрощённый) якобиан гравитационного отклонения — в реальной задаче
// лучше использовать точные модели (SOFA/IAU) или аналитические формулы.
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Более аккуратная оценка якобиана гравитационного отклонения с использованием упрощённой
// реализации на основе данных SOFA/простых аппроксимаций.
// Возвращаем матрицу 3x3, содержащую численные производные выходного направления
// по компонентам входного нормализованного вектора n. На практике здесь можно
// подключить iauLd/iauLdn для точных поправок.
// -----------------------------------------------------------------------------
Eigen::Matrix3d compute_gravitational_deflection_jacobian_sofa(
    ...
    // Подготовка векторов для модели (упрощённый пример)
    ...
    // Простая модель: псевдо-угол отклонения от Солнца (не точная SOFA реализация)
...
// -----------------------------------------------------------------------------
// Якобиан релятивистской аберрации (аналитический вывод). Возвращает 3x3 матрицу
// J_aber = ∂n_aber/∂n, где n_aber — направление после применения формулы Лоренца.
// При отключённой аберрации возвращаем единичную матрицу.
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Гравитационный потенциал от одной точки массы, используется при оценке U_E
// (скалярный потенциал в выбранной точке)
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Релятивистское преобразование GCRS -> BCRS с учётом поправок 1PN (упрощённая форма)
...
// -----------------------------------------------------------------------------
// Простейшая модель гравитационного отклонения света Солнцем (линейная аппрокс.)
...
// -----------------------------------------------------------------------------
// Чтение наблюдений из текстового файла: формат строки ожидается как
// UTC_JD TDB_JD RA_deg DEC_deg OX_km OY_km OZ_km
// Возвращаем вектор Observation, где позиции обсерватории переводятся в метры.
// -----------------------------------------------------------------------------
std::vector<Observation> read_observed_data(const std::string& filename) {
    ...
    while (std::getline(file, line)) {
        ...
        Observation obs;
        obs.time_jd = tdb_jd; // предпочитаем TDB для астрономических вычислений
        obs.time_seconds = jd_to_seconds(tdb_jd);

        // Углы переводим в радианы (RA/DEC в градусах во входном файле)
        obs.ra_observed = ra_deg * DEG_TO_RAD;
        obs.dec_observed = dec_deg * DEG_TO_RAD;

        // Координаты обсерватории в GCRS указаны в километрах в файле → переводим в метры
        obs.obs_position_gcrs = Vec3d(ox_km * 1000.0, oy_km * 1000.0, oz_km * 1000.0);

        // Заполняем скорость обсерватории в GCRS только вращением Земли (приближение)
...
// Возвращаем пару (ra [rad], dec [rad]).
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Вычисление прицельного параметра для гравитационного отклонения (модуль вектора
// перпендикулярного направлению наблюдения). Полезно для оценки величины эффекта.
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Создаём модель системы из нескольких тел: астероид (вариируемый), Солнце, Юпитер, Земля.
// Позиции/скорости для планет заданы в коде (как пример). Возвращаем вектор объектов.
// -----------------------------------------------------------------------------
std::vector<Object> create_model_system(const Vec3d& asteroid_position, const Vec3d& asteroid_velocity) {
    ...
    // Астероид — объект, параметры которого будем менять (is_variable=true)
    Object kobresia("Kobresia", 3.0e16, 0, true);
    kobresia.position = asteroid_position;
    kobresia.velocity = asteroid_velocity;

    // Солнце — фиксированный массив данных (примерные значения в метрах/м/с)
...
// -----------------------------------------------------------------------------
// Инициализация расширенного состояния перед интегрированием: копируем положения
// и скорости из объектов и готовим dr_dbeta/dv_dbeta (по умолчанию dr_dbeta=I для астероида)
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Интегрирование расширённой системы на равномерной сетке времени [t_start, t_end]
// с шагом dt. Результат — вектор состояний states (включая начальное состояние).
// -----------------------------------------------------------------------------
...
// решаем нормальные уравнения (A^T A) Δβ = A^T r с Tikhonov-регуляризацией при необходимости.
// -----------------------------------------------------------------------------
Vec3d solve_inverse_problem(const std::vector<Observation>& observations,
    ...
    // beta — оценка параметров (начальное положение астероида), работаем в метрах
    ...
    for (int iteration = 0; iteration < MAX_ITERATIONS; ++iteration) {
        ...
        // 3) Матрица Якоби A (2*num_obs x 3) и вектор невязок r (2*num_obs)
        ...
        for (int i = 0; i < num_obs; ++i) {
            ...
            // Преобразуем позицию/скорость обсерватории GCRS -> BCRS с релятивистской коррекцией
            ...
            // Поправка за световое время (итеративно) — получаем задержку Δ и время испускания
            ...
            // Положение и скорость астероида в момент испускания (через интерполятор)
            Vec3d ast_pos = asteroid_interp.interpolate(t_emit);
            Vec3d ast_vel = asteroid_vel_interp.interpolate(t_emit);

            // Топоцентрический вектор от наблюдателя к астероиду и его нормировка — основное направление
            ...
            // Вычисляем RA/DEC из скорректированного направления
            ...
            // Невязки — наблюдаемое значение минус вычисленное
            double dra = obs.ra_observed - ra_computed;
            double ddec = obs.dec_observed - dec_computed;

            // Приводим dra к диапазону [-π, π] для корректного усреднения
            ...
            // 2) Якобиан нормировки топоцентрика (∂(r/|r|)/∂r)
            Eigen::Matrix3d J_norm = compute_normalization_jacobian(topocentric);

            // В данной реализации остальные эффекты (гравитация, аберрация) считаем фиксированными
            Eigen::Matrix3d J_dir = Eigen::Matrix3d::Identity();
            J_dir = J_norm * J_dir; // применяем только нормировку

            // Комбинируем геом. якобиан и нормировку → получаем ∂(angles)/∂(topocentric components)
            Eigen::Matrix<double, 2, 3> J_geom_full = J_geom * J_dir;

            // Матрица чувствительности ∂r/∂β в момент испускания (интерполируем)
            ...
            // Если необходимо, корректируем dr_dbeta с учётом зависимости времени испускания
        ...
        // ---------------------------------------------------------------------
        // Решаем нормальные уравнения методом регуляризации Tikhonov, контролируем обусловленность
        // ---------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// main: считываем наблюдения, задаём начальное приближение и запускаем решение
// -----------------------------------------------------------------------------
int main() {
